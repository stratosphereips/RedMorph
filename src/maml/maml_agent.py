    ...
    class PhaseBucket:
        ...
        # optional: pre-adapt metric (only one number per task per epoch is fine)
        pre_adapt_rewards: list[float] = field(default_factory=list)

        def add_episode_batch(self, wins, detected, timeouts, steps_list, returns_list):
            self.wins      += wins
            self.detected  += detected
            self.timeouts  += timeouts
            self.episodes  += len(returns_list)
            self.steps.extend(steps_list)
            self.returns.extend(returns_list)

        def add_pre_adapt(self, value):
            self.pre_adapt_rewards.append(float(value))

        def summary(self):
            import numpy as np
            ep = max(self.episodes, 1)
            win_rate = 100.0 * self.wins / ep
            det_rate = 100.0 * self.detected / ep
            to_rate  = 100.0 * self.timeouts / ep
            avg_ret  = float(np.mean(self.returns)) if self.returns else 0.0
            avg_step = float(np.mean(self.steps))   if self.steps   else 0.0
            pre_adapt = float(np.mean(self.pre_adapt_rewards)) if self.pre_adapt_rewards else 0.0
            return dict(
    ...
    log_filename = f"maml_agent_log_1106.txt"
    ...
    class MAMLAgent(BaseAgent):
        ...
        def __init__(self, host, port, role, policy, inner_lr, outer_lr, inner_steps, max_steps, num_meta_batches, num_eval_batches, meta_batch_size, test_batch_size, eval_meta_batch_size, eval_test_batch_size, task_envs, eval_task_envs):
            ...
            self.task_envs = task_envs or [(self.host, self.port, "default")]   # To address multiple topologies 
            
            self.train_envs = task_envs or [(self.host, self.port, "default")]
            self.eval_envs  = eval_task_envs if (eval_task_envs and len(eval_task_envs) > 0) else self.train_envs

            self._epoch_buckets = None  # reset every epoch
            # file path for per-epoch per-tag metrics (next to your existing eval_metrics.csv)
            self._per_tag_csv_path = None
            self._epoch_idx = None

            self._eval_summary_csv_path = None      # old "log_performance" output
            self._eval_query_csv_path   = None      # per-tag eval_query output


        def sample_task_env(self):  # 1st testing
            return random.choice(self.task_envs)  # (host, port, tag)

        def sample_train_env(self): # Now, make two topologies as training, use another topology as testing
            return random.choice(self.train_envs)

        def sample_eval_env(self):
            return random.choice(self.eval_envs)
        ...
        def play_game(self, params=None, keep_layout=False, traj_logger: 'TrajectoryLogger' = None, traj_meta: dict = None):
            """
            ...
            """
            if keep_layout: # in single task, network layout (topology and IPs) must remain fixed.
                # This doesn't use use_dynamic_addresses. → work as False 
                # self._logger.info(f"[Using Same Network Layout.]")
                observation = self.request_game_reset_keep_layout()

            else: # new task, new batch, network layout should be changed.
                # use_dynamic_addresses = True
                # self._logger.info(f"[Reset the game]")
            ...
            task_id = (traj_meta or {}).get("task", 0)
            trajectory_id = f"epoch-{int(epoch_for_id):04d}_task-{int(task_id):02d}"
        ...
        def collect_episodes(self, total_episodes: int, params=None, keep_layout=False,
            ...
            for i in range(total_episodes):
                ...
                if use_logger is not None:
                    task_id = int((traj_meta or {}).get("task", 0))
                    global_ep_id = task_id * total_episodes + i  # make filenames unique across tasks
                    use_logger.open_episode(epoch=(traj_meta or {}).get("epoch", -1), episode_id=global_ep_id)

                try: 
                    meta = dict(traj_meta or {})
                    task_id = int(meta.get("task", 0))
                    global_ep_id = task_id * total_episodes + i
            ...
            # NEW: per-batch stats payload (keeps both totals and raw lists for aggregation)
            batch_stats = {
                ...
                "step_lists": {
                    "wins": num_win_steps,
                    "detected": num_detected_steps,
                    "timeouts": num_max_steps_steps,
                    "all": num_win_steps + num_detected_steps + num_max_steps_steps,
                },
                "return_lists": {
                    "wins": num_win_returns,
                    "detected": num_detected_returns,
                    "timeouts": num_max_steps_returns,
                    "all": num_win_returns + num_detected_returns + num_max_steps_returns,
                }
            }

            return episodes, training_data_all, all_total_rewards, win_rate, batch_stats
        ...
        def log_performance(self, episode, wins, detected, max_steps,
            ...
            try:
                csv_path = getattr(self, "_eval_summary_csv_path", None)
        ...
        def inner_loop(self, flag_eval, tag=None):
            ...
            # In this first episode, we should change the network layout by setting keep_layout=False.
            # [For log] Before adaptation
            _, _, total_rewards, win_rate, _ = self.collect_episodes(1, params=adapted_params, keep_layout=False)
            before_returns.append(np.mean(total_rewards))
            
            for i in range(self.inner_steps):
                # Collect episodes with support set using current policy.
                episodes, training_data_all, all_total_rewards, win_rate, batch_stats = self.collect_episodes(batch_size, params=adapted_params, keep_layout=True) 
                
                if not flag_eval and tag is not None:
                    b = self._epoch_buckets["train_support"][tag]
                    b.add_episode_batch(
                        wins=batch_stats["wins"],
                        detected=batch_stats["detected"],
                        timeouts=batch_stats["timeouts"],
                        steps_list=batch_stats["step_lists"]["all"],
                        returns_list=batch_stats["return_lists"]["all"],
                    )
        ...
        def outer_loop(self):
            ...
            # task labels for grouping by topology
            task_labels = []

            for i in range(self.num_meta_batches): 

                # === pick a topology for THIS TASK ===
                host, port, tag = self.sample_train_env()
                if (self.host, self.port) != (host, port):
                    print(f"[outer_loop] switching to topology {tag} @ {host}:{port}")
                    switch_env(self, host, port)           # <- reconnect to that coordinator
                
                # inner loop adaptation
                flag_eval = False
                adapted_params, before_returns = self.inner_loop(flag_eval, tag=tag)

                # log pre-adapt for this tag (support notion)
                self._epoch_buckets["train_support"][tag].add_pre_adapt(np.mean(before_returns))

                _, training_data_all, total_rewards, win_rate, batch_stats = self.collect_episodes(
                    self.test_batch_size, params=adapted_params, keep_layout=True
                )
                
                # aggregate into train_query for this tag
                b = self._epoch_buckets["train_query"][tag]
                b.add_episode_batch(
                    wins=batch_stats["wins"],
                    detected=batch_stats["detected"],
                    timeouts=batch_stats["timeouts"],
                ...
                # Build query loss for this task (must retain grad path to self.policy)
                per_episode = self.process_training_data(training_data_all)
                loss = self.compute_policy_loss_vanilla_RL(per_episode)

                all_task_losses.append(loss)

                # --- NEW: remember this task's topology label
                task_labels.append(tag)

            # =========================
            # Task–gradient similarity block (before meta-loss aggregation/backward)
            # =========================

            # 1) One gradient per task (query loss) wrt current policy params
            grads_per_task = []
            ...
            for loss in all_task_losses:
                g = torch.autograd.grad(
                    loss,
                    policy_params,
                    retain_graph=True,    # keep graph so we can compute grads for all 5 tasks
                    create_graph=False,
                    allow_unused=True
                )
                # flatten 
                vecs = [gi.reshape(-1) for gi in g if gi is not None]
                if len(vecs) == 0:
                    flat = torch.zeros(1, device=device)  # or zeros(total_params)
                else:
                    flat = torch.cat(vecs)
                grads_per_task.append(flat)

            # 2) Cosine similarity matrix (5 x 5)
            K = len(grads_per_task)
            cos_mat = torch.zeros(K, K, device=grads_per_task[0].device)
            for i in range(K):
                for j in range(K):
                    cos_mat[i, j] = F.cosine_similarity(grads_per_task[i], grads_per_task[j], dim=0, eps=1e-12)

            # 3) Group by topology pairs (within vs between)
            pair_buckets = defaultdict(list)
            for i in range(K):
                for j in range(i+1, K):
                    key = tuple(sorted([task_labels[i], task_labels[j]]))  # normalize pair key
                    pair_buckets[key].append(cos_mat[i, j].item())

            group_means = {k: (sum(v)/len(v)) for k, v in pair_buckets.items() if len(v) > 0}

            within_vals, between_vals = [], []
            for (a,b), vals in pair_buckets.items():
                if a == b:
                    within_vals += vals
                else:
                    between_vals += vals

            within_mean  = (sum(within_vals)/len(within_vals))   if within_vals  else float('nan')
            between_mean = (sum(between_vals)/len(between_vals)) if between_vals else float('nan')

            # 4) (Light) logging – replace with your logger/MLflow as needed
            if getattr(self, "_sim_csv_path", None):
                os.makedirs(os.path.dirname(self._sim_csv_path), exist_ok=True)
                write_header = not os.path.exists(self._sim_csv_path)
                with open(self._sim_csv_path, "a", newline="", encoding="utf-8") as f:
                    w = csv.writer(f)
                    if write_header:
                        w.writerow(["epoch","pair","mean_cosine","within_mean","between_mean","task_labels"])
                    # write group pairs
                    for pair, val in group_means.items():
                        w.writerow([getattr(self, "_epoch_idx", -1), f"{pair[0]}__{pair[1]}",
                                    f"{val:.6f}", f"{within_mean:.6f}", f"{between_mean:.6f}",
                                    "|".join(task_labels)])

            # ===== continue with your normal meta-update =====
            ...
            avg_before = np.array([acc_before / N], dtype=np.float32)
            avg_after  = acc_after  / N

            return meta_loss.item(), avg_before, avg_after


        def evaluate(self, traj_logger=None, epoch=None, log_one_idx=None, log_on_epochs: set[int] | None = None):
            """
            Evaluation:
            - Always compute metrics and write CSVs (eval_summary.csv, eval_query_per_tag.csv)
            - Only write trajectory files on selected epochs (e.g., epoch 5 and last epoch)
            - On those epochs, log exactly ONE query episode, not all.
            """
            ...
            # Only log trajectories on specific epochs (default: none)
            should_log_traj = (traj_logger is not None and epoch is not None)


            for i in range(self.num_eval_batches):
                host, port, tag = self.sample_eval_env()
                if (self.host, self.port) != (host, port):
                    print(f"[eval] switching to topology {tag} @ {host}:{port}")
                    switch_env(self, host, port)

                # inner loop adaptation (support)
                flag_eval = True
                adapted_params, before_returns = self.inner_loop(flag_eval)
                pre_adapt_rewards.append(np.mean(before_returns))
                self._epoch_buckets["eval_support"][tag].add_pre_adapt(np.mean(before_returns))

                # Trajectory logging policy:
                # - Only on chosen epochs (e.g., {5, meta_epochs})
                # - Only on first eval task (i==0)
                # - Only ONE episode (log_one_idx)
                use_logger = traj_logger if (should_log_traj and i == 0 and log_one_idx is not None) else None

                # Query set evaluation (no further update)
                _, _, total_rewards, win_rate, batch_stats = self.collect_episodes(
                    self.eval_test_batch_size,
                    params=adapted_params,
                    keep_layout=True,
                    traj_logger=use_logger,  # attach only when needed
                    traj_meta={"epoch": epoch, "phase": "eval_query", "task": i} if use_logger else {"epoch": epoch, "phase": "eval_query", "task": i},
                    log_one_idx=log_one_idx if use_logger else None,
                    log_all=False  # do NOT log every episode
                )

                # per-tag evaluation csv row
                if epoch is not None:
                    self.log_eval_metrics_query(epoch=epoch, tag=tag, batch_stats=batch_stats)

                # aggregate into buckets
                b = self._epoch_buckets["eval_query"][tag]
                b.add_episode_batch(
                    wins=batch_stats["wins"],
                    detected=batch_stats["detected"],
                    timeouts=batch_stats["timeouts"],
        ...
        def _flush_per_tag_metrics(self, epoch, mlflow_run=None):
            """
            Summarize self._epoch_buckets and log to MLflow and CSV.
            """
            rows = []
            for phase, tag_map in self._epoch_buckets.items():
                for tag, bucket in tag_map.items():
                    s = bucket.summary()
                    # MLflow (flat names like train_query/win_rate/topo_Small)
                    if mlflow_run is not None:
                        mlflow.log_metric(f"{phase}/win_rate/{tag}",      s["win_rate"],      step=epoch)
                        mlflow.log_metric(f"{phase}/avg_return/{tag}",    s["avg_return"],    step=epoch)
                        mlflow.log_metric(f"{phase}/avg_steps/{tag}",     s["avg_steps"],     step=epoch)
                        mlflow.log_metric(f"{phase}/pre_adapt_avg/{tag}", s["pre_adapt_avg"], step=epoch)
                        mlflow.log_metric(f"{phase}/episodes/{tag}",      s["episodes"],      step=epoch)
                        mlflow.log_metric(f"{phase}/detect_rate/{tag}",   s["detection_rate"],step=epoch)
                        mlflow.log_metric(f"{phase}/timeout_rate/{tag}",  s["timeout_rate"],  step=epoch)
                    # CSV row
                    rows.append([epoch, phase, tag, s["episodes"], f"{s['win_rate']:.3f}",
                                f"{s['detection_rate']:.3f}", f"{s['timeout_rate']:.3f}",
                                f"{s['avg_return']:.4f}", f"{s['avg_steps']:.3f}",
                                f"{s['pre_adapt_avg']:.4f}"])
            # Write/append CSV
            if self._per_tag_csv_path:
                os.makedirs(os.path.dirname(self._per_tag_csv_path), exist_ok=True)
                header = ["epoch", "phase", "tag", "episodes", "win_rate", "detection_rate",
                        "timeout_rate", "avg_return", "avg_steps", "pre_adapt_avg"]
                write_header = not os.path.exists(self._per_tag_csv_path)
                with open(self._per_tag_csv_path, "a", newline="", encoding="utf-8") as f:
                    w = csv.writer(f)
                    if write_header: w.writerow(header)
                    for r in rows: w.writerow(r)

        def _print_epoch_compact(self, epoch: int, include_eval: bool = False):
            def phase_line(phase_name: str):
                tag_map = self._epoch_buckets.get(phase_name, {})
                if not tag_map:
                    return f"{phase_name}: -"
                parts = []
                for tag in sorted(tag_map.keys()):
                    s = tag_map[tag].summary()
                    if phase_name.endswith("support") and s["episodes"] == 0:
                        parts.append(f"{tag}[pre={s['pre_adapt_avg']:.2f}]")
                    else:
                        parts.append(f"{tag}[ep={s['episodes']}, win={s['win_rate']:.1f}%, ret={s['avg_return']:.2f}]")
                return f"{phase_name}: " + "  |  ".join(parts)

            core = f"[Per-Topology @ epoch {epoch}]  {phase_line('train_support')}  ||  {phase_line('train_query')}"
            if include_eval:
                core += f"\n[Eval-Per-Topology @ epoch {epoch}]  {phase_line('eval_support')}  ||  {phase_line('eval_query')}"
            print(core)

        def log_eval_metrics_query(self, epoch: int, tag: str, batch_stats: dict):
            """
            Evaluation-only logger (QUERY episodes).
            Writes one row per (epoch, topology-tag) into eval_metrics.csv and logs MLflow metrics.
            """
            import numpy as np
            import os, csv

            steps_all = batch_stats["step_lists"]["all"]
            rets_all  = batch_stats["return_lists"]["all"]
            steps_win = batch_stats["step_lists"]["wins"]

            episodes = int(batch_stats["episodes"])
            wins     = int(batch_stats["wins"])
            win_rate_pct = 100.0 * wins / max(episodes, 1)

            mean_ret = float(np.mean(rets_all)) if rets_all else 0.0
            std_ret  = float(np.std(rets_all))  if rets_all else 0.0

            mean_steps = float(np.mean(steps_all)) if steps_all else 0.0
            std_steps  = float(np.std(steps_all))  if steps_all else 0.0

            win_steps_mean = float(np.mean(steps_win)) if steps_win else 0.0
            win_steps_std  = float(np.std(steps_win))  if steps_win else 0.0
            win_steps_n    = int(len(steps_win))

            # -------- MLflow (optional, safe if mlflow is active) --------
            try:
                mlflow.log_metric(f"eval_query/win_rate_pct/{tag}", win_rate_pct, step=epoch)
                mlflow.log_metric(f"eval_query/mean_return/{tag}",  mean_ret,     step=epoch)
                mlflow.log_metric(f"eval_query/std_return/{tag}",   std_ret,      step=epoch)
                mlflow.log_metric(f"eval_query/mean_steps/{tag}",   mean_steps,   step=epoch)
                mlflow.log_metric(f"eval_query/std_steps/{tag}",    std_steps,    step=epoch)
                mlflow.log_metric(f"eval_query/win_steps_mean/{tag}", win_steps_mean, step=epoch)
                mlflow.log_metric(f"eval_query/win_steps_std/{tag}",  win_steps_std,  step=epoch)
                mlflow.log_metric(f"eval_query/win_steps_n/{tag}",    win_steps_n,    step=epoch)
                mlflow.log_metric(f"eval_query/episodes/{tag}",       episodes,       step=epoch)
            except Exception:
                pass

            # -------- CSV row (evaluation report file) --------
            csv_path = getattr(self, "_eval_query_csv_path", None)
            if not csv_path:
                return
            ...
            header = [
                "epoch", "tag", "eval_query_episodes",
                "win_rate_pct",
                "mean_return", "std_return",
                "mean_steps", "std_steps",
                "win_steps_n", "win_steps_mean", "win_steps_std"
            ]
            ...
            with open(csv_path, "a", newline="", encoding="utf-8") as f:
                ...
                w.writerow([
                    epoch, tag, episodes,
                    f"{win_rate_pct:.3f}",
                    f"{mean_ret:.6f}", f"{std_ret:.6f}",
                    f"{mean_steps:.6f}", f"{std_steps:.6f}",
                    win_steps_n, f"{win_steps_mean:.6f}", f"{win_steps_std:.6f}"
                ])
    ...
    def switch_env(agent, host, port):
        ...
        agent.reconnect(host, port)
        rh, rp = agent.socket.getpeername()
        # print(f"[Switch ENV] New Connection to {host}:{port} / ACTIVE TCP PEER: {rh}:{rp}")
    ...
    if __name__ == "__main__":
        ...
        logging.basicConfig(filename=path.join(args.logdir, "maml_agent_2026.log"),
        ...
        # task_envs = [
        #     ("127.0.0.3", 9000, "topo_Normal"),
        #     ("127.0.0.4", 9000, "topo_Small"),
        #     ("127.0.0.5", 9000, "topo_Tiny"),
        # ]
        ...
        task_envs_eval = [
            ("127.0.0.3", 9000, "topo_Normal")   # unseen in training
        ]
        ...
        agent = MAMLAgent(
            ...
            # task_envs=task_envs
            task_envs=task_envs_train,
            eval_task_envs=task_envs_eval
        )
        ...
        with mlflow.start_run(run_name=args.experiment_name) as run:
            ...
            agent._eval_summary_csv_path = path.join(traj_base_dir, "results", "eval_summary.csv")
            agent._eval_query_csv_path   = path.join(traj_base_dir, "results", "eval_query_per_tag.csv")
            makedirs(path.join(traj_base_dir, "results"), exist_ok=True)
            ...
            traj_logger.write_manifest({
                ...
                "eval_envs": agent.eval_envs
            })

            agent._per_tag_csv_path = path.join(traj_base_dir, "results", "per_tag_metrics.csv")
            agent._sim_csv_path = path.join(traj_base_dir, "results", "task_similarity.csv")
            ...
            if args.testing:
                ...
                if args.eval_checkpoint:
                    ...
                    epoch_to_report = int(ckpt.get("epoch", args.meta_epochs))
                    print(f"[Eval-Only] Loaded checkpoint from {args.eval_checkpoint} (epoch={epoch_to_report}, best_metric={ckpt.get('best_metric')})")
                else:
                    epoch_to_report = args.meta_epochs

                log_epochs = {epoch_to_report}  # in eval-only, just log once
                chosen_idx = eval_test_batch_size - 1

                eval_avg_before, win_rate, eval_avg_reward = agent.evaluate(
                    traj_logger=traj_logger,
                    epoch=epoch_to_report,
                    log_one_idx=chosen_idx,
                    log_on_epochs=log_epochs
                )

                mlflow.log_metric("eval_pre_adapt_avg_reward", eval_avg_before)
                mlflow.log_metric("eval_win_rate", win_rate)
                mlflow.log_metric("eval_avg_reward", eval_avg_reward)
                ...
                for epoch in range(1, args.meta_epochs + 1):
                    ...
                    agent._epoch_buckets = {
                        "train_support": defaultdict(PhaseBucket),
                        "train_query":   defaultdict(PhaseBucket),
                        "eval_support":  defaultdict(PhaseBucket),
                        "eval_query":    defaultdict(PhaseBucket),
                    }
                    agent._epoch_idx = epoch
                    ...
                    # after training meta_loss logs (and optionally after eval for that epoch)
                    agent._flush_per_tag_metrics(epoch, mlflow_run=mlflow)  # mlflow comes from context
                    ...
                    agent._print_epoch_compact(epoch)
                    ...
                    if epoch % 5 == 0 or epoch == args.meta_epochs or epoch == early_stopping:
                        # --- 1) Meta-Loss Plot ---
